...
- **MCP served from the daemon (v2)**: `grepai mcp-serve` now works for engine:v2 repos — `grepai_search`, `grepai_trace_callers|callees|graph` (v1-parity output via the shared assembly), and `grepai_index_status` (v2 shape) route to the grepaid daemon per tool call (query-only; lazy daemon start; robust to daemon restarts). v1-only tools (`grepai_refs_*`, `grepai_rpg_*`) and workspace parameters reject loudly per call. Also closes the unscoped-mode gap from the #8 review: a v1-mode server now rejects tool-call-selected workspaces containing engine:v2 members. Closes #10.

- **v1-parity trace output (v2)**: engine:v2 trace now renders through v1's own output layer over a genuine `trace.TraceResult` — JSON, `--toon`, `--ui`, and text output match v1 fast mode (resolved caller/callee symbols, call-site `context`, `receiver`/`exported`/`language`, graph node table). Migration 0004 widens the symbol tables; the extractor-version bump re-extracts existing catalogs on the next daemon restart (no re-embedding). Not replicated: `feature_path` (RPG is not daemon-served), `--mode precise`, `--workspace`/`--project` (still rejected loudly). Closes #20.

- **Trace under the v2 daemon**: `grepai trace callers|callees|graph` now works for engine:v2 repos — symbols and call edges are extracted in the daemon's build path (regex extractor; tree-sitter behind the `treesitter` build tag), stored artifact-scoped in the per-repo catalog (migration 0003), and served over RPC from the active view. Existing catalogs are backfilled on daemon start (CPU-only, one repo at a time, zero re-embedding); trace warns while coverage is still building. Closes #9.
